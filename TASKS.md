...
- [x] تثبيت `nwidart/laravel-modules`.
- [x] إنشاء **Core Module** (Shared Kernel).
- [x] إنشاء ملفات القاعدة (BaseDTO, BaseAction, BaseService).

### 1.2 استكمال نظام الفروع (Full Branches System)
- [x] بناء CRUD الفروع وإدارة صلاحيات الموظفين لكل فرع.
- [x] اختبار عزل البيانات (Branch Isolation) بين مستخدمين من فروع مختلفة.

### 1.3 استراتيجية الفهرسة (Database Indexing)
- [x] إضافة Indexes للحقول: `tenant_id`, `branch_id`, `status`, `created_at`.

### 1.4 التحكم في ظهور المنتجات (Visibility Controls)
- [x] إضافة حقول `is_active_in_store` و `is_active_in_sales` لجدول المنتجات.
- [x] تعديل الـ Queries الخاصة بالمتجر لتشمل المنتجات المفعلة للمتجر فقط.
- [x] تعديل الـ Queries الخاصة بالـ POS/Sales لتشمل المنتجات المفعلة للمبيعات فقط.
...
- [x] بناء دليل الحسابات (COA) ومحرك الترحيل التلقائي (تم النقل لموديول Accounting).
...
- [x] ترحيل موديول المخازن (Inventory) بالكامل.
...
| :--- | :--- | :--- |
| **0 - Stabilization** | ✅ مكتملة | 100% |
| **1 - Modularization** | ✅ مكتملة | 100% |
| **2 - Commercial** | ⏳ في الانتظار | 0% |
| **3 - Accounting** | 🚧 جاري العمل | 40% |
| **4 - Inventory** | 🚧 جاري العمل | 50% |
| **باقي المراحل** | ⏳ في الانتظار | 0% |

---

_آخر تحديث: 12 مايو 2026_
