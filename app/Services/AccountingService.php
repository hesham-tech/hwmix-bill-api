...
class AccountingService
{
    ...
    public function collectPayment(User $staff, User $party, float $amount, array $options = []): void
    {
        DB::transaction(function () use ($staff, $party, $amount, $options) {
            ...
            if (!$companyId) {
                throw new \Exception("لا يمكن تحديد الشركة لإتمام عملية التحصيل.");
            }

            $cashBoxId = $options['cash_box_id'] ?? null;
            $description = $options['notes'] ?? $options['description'] ?? 'تحصيل دفعة نقدية';

            $this->recordPayment($companyId, $staff, $party, $amount, 'in', [
                'cash_box_id' => $cashBoxId,
                'description' => $description,
                'installment_id' => $options['installment_id'] ?? null,
            ]);
        });
    }
}
