...
## 🔟 قواعد العلاقات التجارية والقدرات (Capability-Based Business Relations)

1. **يمنع منعاً باتاً** استخدام المقارنات النصية المباشرة للأدوار أو أنواع العلاقات مثل `employee` أو `customer` أو `supplier` لتحديد منطق العمل (Business Logic) في النظام.
2. **المرجع الوحيد** للتحقق من الصلاحيات والقدرات التشغيلية للمستخدم هو دالة `hasCapability(CapabilityCode::CUSTODY_CODE, $companyId)` الموجودة في السمة `HasBusinessCapabilities` بموديل `User`.
3. **سجل الثوابت (Registry):** يجب دائماً استخدام كلاس `App\Enums\CapabilityCode` لتعريف واستدعاء كودات القدرات لتجنب الأخطاء الإملائية والسلاسل النصية العشوائية.
4. **التجهيز التلقائي (Provisioning):** أي عملية ربط مستخدم بشركة (CompanyUser) يجب أن يتم معالجة آثارها الجانبية (مثل تهيئة الخزنة النقدية أو الذمة المالية) تلقائياً في `CompanyUserObserver` بناءً على قدرات المستخدم النشطة فقط.
5. **القدرات معرفة برمجياً (System-Defined):** القدرات (`capabilities`) هي خصائص ثابتة بالنظام لا يمكن إضافتها ديناميكياً من واجهة الإدارة بدون كود يدعمها، بينما أنواع العلاقات (`relation_types`) هي بيانات ديناميكية قابلة للإضافة والتعديل والربط بالقدرات.

---

*آخر تحديث: يوليو 2026 — يجب مراجعة هذا الملف عند إضافة معايير جديدة*
