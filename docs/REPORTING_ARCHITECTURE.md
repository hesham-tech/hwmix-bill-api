> **الهدف:** بناء نظام تقارير محاسبي احترافي يعتمد على أدق المعايير (Accounting Standards) ويضمن ثبات البيانات التاريخية وسرعة الأداء.

---

## 💎 مبادئ النظام الاحترافي (Professional Standards)

لضمان عدم وجود أخطاء محاسبية، يجب أن يتبع النظام القواعد التالية:

1. **اللقطة التاريخية (Cost Snapshotting):** لا يتم حساب الربح بناءً على سعر التكلفة _الحالي_ للمنتج، بل بناءً على سعر التكلفة _وقت البيع_. (تمت إضافة حقل `cost_price` لجدول البنود).
2. **نظام القيد المزدوج (Double-Entry Ledger):** كل عملية مالية يجب أن تُسجل كحركة في "دفتر الأستاذ" لضمان توازن الميزانية (Assets = Liabilities + Equity).
3. **فصل التكاليف (COGS vs Expenses):** التفريق بين "تكلفة البضاعة" (مباشرة) وبين "المصاريف التشغيلية" (إيجار، رواتب).
4. **إغلاق الفترات (Period Closing):** القدرة على إغلاق شهر محاسبي لمنع التعديل عليه وضمان ثبات التقارير المستخرجة سابقاً.
...
-- 3. جدول المصاريف (Expenses)
CREATE TABLE expenses (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    company_id BIGINT NOT NULL,
    category_id BIGINT NOT NULL, -- (رواتب، إيجار، كهرباء، إلخ)
...
-- 4. جدول القيد المالي (Financial Ledger) - المصدر النهائي للحقيقة
CREATE TABLE financial_ledger (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    company_id BIGINT NOT NULL,
    account_type VARCHAR(50), -- (Sales, COGS, Expenses, Tax)
    debit DECIMAL(15,2) DEFAULT 0,
    credit DECIMAL(15,2) DEFAULT 0,
    reference_type VARCHAR(50), -- (Invoice, Expense, Payment)
    reference_id BIGINT,
    date DATE NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
